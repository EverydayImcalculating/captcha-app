...
export function RawDataTable() {
  ...
  const downloadCSV = () => {
    const headers = ['Round', 'Type', 'Time (ms)', 'Accuracy', 'Frustration'];
    const rows = results.map((r) => [
      r.round,
      r.type,
      r.timeTaken,
      r.accuracy ? 'Pass' : 'Fail',
      r.frustrationScore || 'N/A',
    ]);
  ...
  return (
    <Card className="w-full border-4 border-white/50 shadow-cute bg-white/80 backdrop-blur-sm rounded-3xl">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-2xl font-bold text-primary">Raw Test Data</CardTitle>
        <Button
          onClick={downloadCSV}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-primary/20">
                <th className="text-left p-2 font-bold text-foreground">Round</th>
                <th className="text-left p-2 font-bold text-foreground">Type</th>
                <th className="text-right p-2 font-bold text-foreground">Time (s)</th>
                <th className="text-center p-2 font-bold text-foreground">Result</th>
                <th className="text-center p-2 font-bold text-foreground">Frustration</th>
              </tr>
            </thead>
            <tbody>
              {results.map((result, idx) => (
                <tr key={idx} className="border-b border-muted/30 hover:bg-secondary/10 transition-colors">
